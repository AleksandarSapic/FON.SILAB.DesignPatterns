...
// Promenljivo!!!  
    void napuniDomenskiObjekatIzGrafickogObjekta() {
        ...
        let.setStanje(ef.getPanel().getStanje());
    }

// Promenljivo!!!
    @Override
    public void napuniGrafickiObjekatIzDomenskogObjekta(DKLet let) {
        ...
        ef.getPanel().setStanje(let.getStanje());
...
// Promenljivo!!!
    ...
    Stanje odrediStanjeIspitnePrijave() {
        bbp.makeConnection();
        Stanje st = null;
        DKLet let1 = (DKLet) bbp.findRecord(let);
        bbp.closeConnection();
        if (let1 != null) {
            if (let1.getStanje().equals("obradjen")) {
                st = new Obradjen(this);
            }
            if (let1.getStanje().equals("neobradjen")) {
                st = new Neobradjen(this);
            }
            if (let1.getStanje().equals("storniran")) {
                st = new Storniran(this);
            }
        } else {
...
class OsluskivacObradi implements ActionListener {
    ...
    OsluskivacObradi(Kontroler1 kon1) {
        kon = kon1;
    }
...
class OsluskivacStorniraj implements ActionListener {
    ...
    OsluskivacStorniraj(Kontroler1 kon1) {
        kon = kon1;
    }
